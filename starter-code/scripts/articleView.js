...
articleView.create = function(e) {
  ...
  articleData.authorUrl = $form.find('#authorUrl').val();
  articleData.category = $form.find('#category').val();
  articleData.publisedOn = $form.find('#publised').is(':checked')? new Date() : undefined;
  /*if ($form.find('#publised').is())(':checked') {
  ...
  $('#articles').prepand(template(articleData)).attr('id', 'new-article-preview');
